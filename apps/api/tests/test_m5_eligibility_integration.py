...
from app.infrastructure.models.students import Institution, AcademicYear, User, Student, StudentRevision, Department, Degree, AcademicRecord, Backlog, BacklogEvent
from app.infrastructure.models.recruitment import Company, CompanyRole, Drive, Opportunity, Offer, OfferEvent
from app.infrastructure.models.policy import PolicyVersion, PolicyActivation, PolicyRule, RequirementVersion, Criterion
...
async def ay(db_session: AsyncSession, inst):
    ay = AcademicYear(institution_id=inst.id, code=f"AY25_M5_{uuid4().hex[:8]}", starts_at=datetime.now(timezone.utc) - timedelta(days=30), ends_at=datetime.now(timezone.utc) + timedelta(days=365), source_type="SYSTEM", source_reference="test")
    db_session.add(ay)
    await db_session.commit()
...
async def client(inst, db_session):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test", headers={"Authorization": "Bearer fake"}) as c:
        yield c

async def setup_base_data(db_session, inst, ay):
    # Setup Company, Drive, Opportunity
    comp = Company(institution_id=inst.id, code=f"COMP_{uuid4().hex[:8]}", source_type="SYSTEM", source_reference="test")
    db_session.add(comp)
    await db_session.flush()
    
    role = CompanyRole(institution_id=inst.id, company_id=comp.id, code=f"R_{uuid4().hex[:4]}", title="SDE", source_type="SYSTEM", source_reference="test")
    db_session.add(role)
    
    drive = Drive(institution_id=inst.id, company_id=comp.id, academic_year_id=ay.id, status="ANNOUNCED", announced_at=datetime.now(timezone.utc), source_type="SYSTEM", source_reference="test")
    db_session.add(drive)
    await db_session.flush()
    
    opp = Opportunity(institution_id=inst.id, company_id=comp.id, role_id=role.id, drive_id=drive.id, status="OPEN", source_type="SYSTEM", source_reference="test")
    db_session.add(opp)
    
    # Setup Student
    u = User(institution_id=inst.id, login=f"student_{uuid4().hex[:8]}@example.com", status="ACTIVE", source_type="SYSTEM", source_reference="test")
    db_session.add(u)
    await db_session.flush()
    
    student = Student(institution_id=inst.id, roll_number=f"R_{uuid4().hex[:8]}", user_id=u.id, source_type="SYSTEM", source_reference="test")
    db_session.add(student)
    await db_session.flush()
    
    s_rev = StudentRevision(institution_id=inst.id, student_id=student.id, version=1, full_name="Test Student", source_type="SYSTEM", source_reference="test", effective_at=datetime.now(timezone.utc))
    db_session.add(s_rev)
    
    from app.infrastructure.models.students import GradingScale
    scale = GradingScale(institution_id=inst.id, code="10_POINT", minimum=0, maximum=10, source_type="SYSTEM", source_reference="test")
    db_session.add(scale)
    await db_session.flush()
    
    ac = AcademicRecord(institution_id=inst.id, student_id=student.id, version=1, cgpa=8.5, cgpa_state="KNOWN", sslc_state="UNKNOWN", hsc_state="UNKNOWN", diploma_state="UNKNOWN", scale_id=scale.id, source_type="SYSTEM", source_reference="test", effective_at=datetime.now(timezone.utc))
    db_session.add(ac)
    
    # Add a backlog
    b = Backlog(institution_id=inst.id, student_id=student.id, obligation_key="CS101", source_type="SYSTEM", source_reference="test")
    db_session.add(b)
    await db_session.flush()
    be = BacklogEvent(institution_id=inst.id, backlog_id=b.id, version=1, kind="OPENED", source_type="SYSTEM", source_reference="test", effective_at=datetime.now(timezone.utc))
    db_session.add(be)
    
    # Add an offer
    o = Offer(institution_id=inst.id, student_id=student.id, company_id=comp.id, role_id=role.id, academic_year_id=ay.id, context="Campus", source_type="SYSTEM", source_reference="test")
    db_session.add(o)
    await db_session.flush()
    from app.infrastructure.models.recruitment import Compensation
    comp_terms = Compensation(institution_id=inst.id, original_text="10LPA", currency="INR", period="ANNUAL", basis="TOTAL_CTC", shape="EXACT", amount=1000000, comparison_state="UNRESOLVED", review_reason="test", source_type="SYSTEM", source_reference="test")
    db_session.add(comp_terms)
    await db_session.flush()

    oe = OfferEvent(institution_id=inst.id, offer_id=o.id, version=1, kind="ACCEPTED", compensation_id=comp_terms.id, source_type="SYSTEM", source_reference="test", effective_at=datetime.now(timezone.utc))
    db_session.add(oe)
    
    from app.infrastructure.models.policy import Requirement
    req = Requirement(institution_id=inst.id, opportunity_id=opp.id, source_type="SYSTEM", source_reference="test")
    db_session.add(req)
    await db_session.flush()

    rv = RequirementVersion(institution_id=inst.id, requirement_id=req.id, version=1, compensation_id=comp_terms.id, source_type="SYSTEM", source_reference="test", effective_at=datetime.now(timezone.utc))
    db_session.add(rv)
    await db_session.flush()
    
    rc = Criterion(institution_id=inst.id, requirement_version_id=rv.id, code="academic.cgpa", applicability="REQUIRED", operator=">=", operand={"value": 8.0}, source_type="SYSTEM", source_reference="test")
    db_session.add(rc)
...
async def setup_policy(db_session, inst, ay, scope="ALL", status="ACTIVE", starts=None, ends=None):
    ...
    policy = Policy(institution_id=inst.id, code=f"P_{uuid4().hex[:8]}", name="Test Policy", source_type="SYSTEM", source_reference="test")
    db_session.add(policy)
    await db_session.flush()

    pv = PolicyVersion(institution_id=inst.id, policy_id=policy.id, version=1, academic_year_id=ay.id, scope=scope, status="DRAFT", schema_version=1, definition={}, source_type="SYSTEM", source_reference="test", effective_at=datetime.now(timezone.utc))
    db_session.add(pv)
    await db_session.flush()
    
    pa = PolicyActivation(institution_id=inst.id, policy_version_id=pv.id, academic_year_id=ay.id, scope=scope, starts_at=starts, ends_at=ends, source_type="SYSTEM", source_reference="test")
    db_session.add(pa)
    
    pr = PolicyRule(institution_id=inst.id, policy_version_id=pv.id, code="placement_history.active_offer_count", definition={"operator": "<=", "operand": {"value": 0}, "type": "constraint"}, source_type="SYSTEM", source_reference="test")
    db_session.add(pr)
...
async def test_active_policy_selection(db_session, inst, ay, client):
    ...
    resp = await client.post("/api/v1/eligibility/evaluate", json={
...
async def test_missing_authoritative_fact_becomes_unknown(db_session, inst, ay, client):
    ...
    # Nullify cgpa
    stmt = text(f"UPDATE student_academic_records SET cgpa_state = 'UNKNOWN', cgpa = NULL WHERE student_id = '{student.id}'")
...
async def test_evaluation_idempotency(db_session, inst, ay, client):
    ...
    resp1 = await client.post("/api/v1/eligibility/evaluate", json={
        "student_id": str(student.id),
        "opportunity_id": str(opp.id),
        "requirement_version_id": str(rv.id)
    })
    ...
    resp2 = await client.post("/api/v1/eligibility/evaluate", json={
        "student_id": str(student.id),
        "opportunity_id": str(opp.id),
        "requirement_version_id": str(rv.id)
    })
...
async def test_engine_evaluates_domain_criteria_with_snapshot_isolation(db_session, inst, ay):
    ...
    p_rules_dicts = [{"field": "placement_history.active_offer_count", "operator": "<=", "operand": {"value": 0}}]
    r_criteria_dicts = [{"field": "academic.cgpa", "operator": ">=", "operand": {"value": 8.0}, "applicability": "REQUIRED"}]
...
async def test_concurrent_evaluation(db_engine, inst, ay):
    ...
    async with AsyncClient(transport=ASGITransport(app=app1), base_url="http://test", headers={"Authorization": "Bearer fake"}) as client1, \
               AsyncClient(transport=ASGITransport(app=app2), base_url="http://test", headers={"Authorization": "Bearer fake"}) as client2:
