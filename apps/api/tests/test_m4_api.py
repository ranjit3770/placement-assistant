...
async def app_client(inst, db_session, monkeypatch):
    ...
    u = User(
        id=principal.sub,
        institution_id=inst.id,
        login=f"coord_{uuid4().hex[:8]}@test.com",
        source_type="SYSTEM",
        source_reference="test",
    )
    db_session.add(u)
...
async def student_client(inst, db_session):
    ...
    u = User(
        id=principal.sub,
        institution_id=inst.id,
        login=f"student_{uuid4().hex[:8]}@test.com",
        source_type="SYSTEM",
        source_reference="test",
    )
    db_session.add(u)
...
async def test_policy_creation_and_lifecycle(
    ...
    res = await app_client.post(
        "/api/v1/policies",
        json={"code": "PLACEMENT-2026", "name": "Placement Rules 2026"},
        headers=auth_headers,
    )
    assert res.status_code == 201
    ...
    # Add Rule in DRAFT
    res = await app_client.post(
        f"/api/v1/policies/versions/{version_id}/rules",
        json={"code": "CGPA_REQ", "definition": {"min_cgpa": 7.0}},
        headers=auth_headers,
    )
    assert res.status_code == 201
    ...
    res = await app_client.post(
        f"/api/v1/policies/versions/{version_id}/review",
        json={"reason": "Skip processing"},
        headers=auth_headers,
    )
    ...
    res = await app_client.post(
        f"/api/v1/policies/versions/{version_id}/processing",
        json={"reason": "Start processing"},
        headers=auth_headers,
    )
    ...
    res = await app_client.post(
        f"/api/v1/policies/versions/{version_id}/rules",
        json={"code": "BACKLOG_REQ", "definition": {"max_backlogs": 0}},
        headers=auth_headers,
    )
    assert res.status_code == 201
    ...
    res = await app_client.post(
        f"/api/v1/policies/versions/{version_id}/rules",
        json={"code": "INVALID_RULE", "definition": {}},
        headers=auth_headers,
    )
    assert res.status_code == 400
    ...
    res = await student_client.post(
        f"/api/v1/policies/versions/{version_id}/approve",
        json={"reason": "Student trying to approve"},
    )
    ...
    res = await app_client.post(
        f"/api/v1/policies/versions/{version_id}/activate",
        json={
    ...
    await app_client.post(
        f"/api/v1/policies/versions/{v2_id}/processing", json={"reason": "1"}, headers=auth_headers
    )
    await app_client.post(
        f"/api/v1/policies/versions/{v2_id}/review", json={"reason": "2"}, headers=auth_headers
    )
    await app_client.post(
        f"/api/v1/policies/versions/{v2_id}/approve", json={"reason": "3"}, headers=auth_headers
    )

    async with db_session.begin_nested():
        res = await app_client.post(
            f"/api/v1/policies/versions/{v2_id}/activate",
            json={
                "reason": "Overlap",
    ...
    v2_db = (
        await db_session.execute(select(PolicyVersion).where(PolicyVersion.id == v2_id))
    ).scalar_one()
    ...
    activations = (
        (
            await db_session.execute(
                select(PolicyActivation).where(PolicyActivation.policy_version_id == v2_id)
            )
        )
        .scalars()
        .all()
    ...
    events = (
        (
            await db_session.execute(
                select(PolicyEvent).where(PolicyEvent.policy_version_id == v2_id)
            )
        )
        .scalars()
        .all()
    ...
    # Archive original
    res = await app_client.post(
        f"/api/v1/policies/versions/{version_id}/archive",
        json={"reason": "End of year"},
        headers=auth_headers,
    )
    assert res.status_code == 200
...
async def test_concurrent_policy_activation(db_engine):
    ...
    results = await asyncio.gather(
        activate_version(v1_id), activate_version(v2_id), return_exceptions=True
    )
...
async def test_concurrent_api_activation(db_engine, inst, ay):
    ...
    res = await client1.post(
        "/api/v1/policies",
        json={"code": f"API-CONC-{uuid4().hex[:8]}", "name": "API Concurrency Policy"},
        headers=auth_headers,
    )
    assert res.status_code == 201
    policy_id = res.json()["id"]
    ...
    await client1.post(
        f"/api/v1/policies/versions/{v1_id}/processing", json={"reason": "1"}, headers=auth_headers
    )
    await client1.post(
        f"/api/v1/policies/versions/{v1_id}/review", json={"reason": "2"}, headers=auth_headers
    )
    await client1.post(
        f"/api/v1/policies/versions/{v1_id}/approve", json={"reason": "3"}, headers=auth_headers
    )
    ...
    await (
        session1.commit()
    )  # commit the setup to the real DB so session2 can see the APPROVED versions
    ...
    async def activate_v1():
        async with session1.begin():
            res = await client1.post(
                f"/api/v1/policies/versions/{v1_id}/activate", json=payload, headers=auth_headers
            )
    ...
    async def activate_v2():
        async with session2.begin():
            res = await client2.post(
                f"/api/v1/policies/versions/{v2_id}/activate", json=payload, headers=auth_headers
            )
            return ("v2", res.status_code)

    results = await asyncio.gather(activate_v1(), activate_v2(), return_exceptions=True)
    ...
    for r in results:
        if isinstance(r, Exception):
            if (
                isinstance(r, (PendingRollbackError, IntegrityError))
                or "PendingRollbackError" in str(r)
                or "IntegrityError" in str(r)
                or "409" in str(r)
            ):
    ...
    async with async_session() as check_session:
        v_db = (
            await check_session.execute(
                select(PolicyVersion).where(PolicyVersion.id == losing_version_id)
            )
        ).scalar_one()
            ...
            (
                await check_session.execute(
                    select(PolicyActivation).where(
                        PolicyActivation.policy_version_id == losing_version_id
                    )
                )
            )
            .scalars()
        ...
        events = (
            (
                await check_session.execute(
                    select(PolicyEvent).where(PolicyEvent.policy_version_id == losing_version_id)
                )
            )
            .scalars()
            .all()
