...
async def ay(db_session: AsyncSession, inst):
    ...
    ay = AcademicYear(institution_id=inst.id, code="AY25", starts_at=datetime.now(timezone.utc), ends_at=datetime.now(timezone.utc) + timedelta(days=365), source_type="SYSTEM", source_reference="test")
    db_session.add(ay)
    await db_session.commit()
...
async def app_client(inst, db_session, monkeypatch):
    ...
    principal = Principal(sub=str(uuid4()), user_id=uuid4(), institution_id=inst.id, role="COORDINATOR")
...
async def test_companies_api(app_client):
    ...
    # Create role
    res = await app_client.post(f"/api/v1/companies/{company_id}/roles", json={"code": "SWE", "title": "Software Engineer"})
    assert res.status_code == 201
    role_id = res.json()["id"]
...
async def test_opportunities_api(app_client, ay):
    ...
    res = await app_client.post("/api/v1/companies", json={"code": company_code, "name": "Microsoft"})
    company_id = res.json()["id"]
    res = await app_client.post(f"/api/v1/companies/{company_id}/roles", json={"code": "SWE", "title": "Software Engineer"})
    role_id = res.json()["id"]

    # Create drive
    res = await app_client.post(f"/api/v1/companies/{company_id}/drives", json={"academic_year_id": str(ay.id)})
    assert res.status_code == 201
    drive_id = res.json()["id"]

    # Create opportunity
    res = await app_client.post(f"/api/v1/companies/{company_id}/opportunities", json={
        "role_id": role_id,
        "drive_id": drive_id,
    })
    assert res.status_code == 201
...
async def test_requirements_api(app_client, ay):
    ...
    company_id = res.json()["id"]
    res = await app_client.post(f"/api/v1/companies/{company_id}/roles", json={"code": "SDE1", "title": "Software Dev"})
    role_id = res.json()["id"]
    res = await app_client.post(f"/api/v1/companies/{company_id}/drives", json={"academic_year_id": str(ay.id)})
    drive_id = res.json()["id"]
    res = await app_client.post(f"/api/v1/companies/{company_id}/opportunities", json={
    ...
    res = await app_client.post(f"/api/v1/requirements/versions/{req_v_id}/criteria", json=crit_data)
    assert res.status_code == 201
    crit_id = res.json()["id"]

    # 3. Add member
    res = await app_client.post(f"/api/v1/requirements/criteria/{crit_id}/members", json={"value": "B.Tech CSE"})
    assert res.status_code == 201
...
async def test_tenant_isolation(app_client, db_session, inst, ay):
    ...
    res = await app_client.post("/api/v1/companies", json={"code": company_code, "name": "Company A"})
    assert res.status_code == 201
    company_id = res.json()["id"]
...
async def test_company_role_ownership(app_client, ay):
    ...
    # Company A
    res = await app_client.post("/api/v1/companies", json={"code": f"CA_{uuid4().hex[:8]}", "name": "Company A"})
    comp_a = res.json()["id"]
    res = await app_client.post(f"/api/v1/companies/{comp_a}/roles", json={"code": "R_A", "title": "Role A"})
    role_a = res.json()["id"]
    
    # Company B
    res = await app_client.post("/api/v1/companies", json={"code": f"CB_{uuid4().hex[:8]}", "name": "Company B"})
    comp_b = res.json()["id"]
    res = await app_client.post(f"/api/v1/companies/{comp_b}/roles", json={"code": "R_B", "title": "Role B"})
    role_b = res.json()["id"]
    
    res = await app_client.post(f"/api/v1/companies/{comp_a}/drives", json={"academic_year_id": str(ay.id)})
    drive_a = res.json()["id"]

    # Attempt to use Role B on Company A
    res = await app_client.post(f"/api/v1/companies/{comp_a}/opportunities", json={
        "role_id": role_b,
        "drive_id": drive_a
    })
...
async def test_opportunity_close_idempotency(app_client, ay):
    ...
    res = await app_client.post("/api/v1/companies", json={"code": f"LIFE_{uuid4().hex[:8]}", "name": "Life"})
    comp_id = res.json()["id"]
    res = await app_client.post(f"/api/v1/companies/{comp_id}/roles", json={"code": "R", "title": "Role"})
    role_id = res.json()["id"]
    res = await app_client.post(f"/api/v1/companies/{comp_id}/drives", json={"academic_year_id": str(ay.id)})
    drive_id = res.json()["id"]
    
    res = await app_client.post(f"/api/v1/companies/{comp_id}/opportunities", json={
        "role_id": role_id,
        "drive_id": drive_id
    })
    opp_id = res.json()["id"]
...
async def test_requirement_transaction_rollback(app_client, db_session, ay):
    ...
    res = await app_client.post("/api/v1/companies", json={"code": f"RB_{uuid4().hex[:8]}", "name": "Rollback"})
    comp_id = res.json()["id"]
    res = await app_client.post(f"/api/v1/companies/{comp_id}/roles", json={"code": "R", "title": "Role"})
    role_id = res.json()["id"]
    res = await app_client.post(f"/api/v1/companies/{comp_id}/drives", json={"academic_year_id": str(ay.id)})
...
async def test_published_version_mutation_rejection(app_client, ay):
    ...
    res = await app_client.post("/api/v1/companies", json={"code": f"PUB_{uuid4().hex[:8]}", "name": "Publish"})
    comp_id = res.json()["id"]
    res = await app_client.post(f"/api/v1/companies/{comp_id}/roles", json={"code": "R", "title": "Role"})
    role_id = res.json()["id"]
    res = await app_client.post(f"/api/v1/companies/{comp_id}/drives", json={"academic_year_id": str(ay.id)})
