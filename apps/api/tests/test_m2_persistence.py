...
async def test_tenant_isolation_reads(
    db_session: AsyncSession, inst_a: Institution, inst_b: Institution
):
...
async def test_append_only_event_protection(db_session: AsyncSession, inst_a: Institution):
    ...
    student = await student_repo.create(
        roll_number="A2", source_type="SYSTEM", source_reference="seed"
    )

    backlog = Backlog(
        institution_id=inst_a.id,
        student_id=student.id,
        obligation_key="MAT101",
        source_type="SYSTEM",
        source_reference="seed",
    )
...
async def test_active_definition_immutability(
    db_session: AsyncSession, inst_a: Institution, ay_a: AcademicYear
):
    inst_id = inst_a.id
    ay_id = ay_a.id
    repo = PolicyVersionRepository(db_session, inst_id)
    policy = Policy(
        institution_id=inst_id, code="P1", name="Pol", source_type="SYSTEM", source_reference="seed"
    )
    ...
    rule_draft = PolicyRule(
        institution_id=inst_id,
        policy_version_id=pv_draft_id,
        code="R2",
        definition={"x": 2},
        source_type="SYSTEM",
        source_reference="seed",
...
async def test_published_requirement_immutability(
    db_session: AsyncSession, inst_a: Institution, ay_a: AcademicYear
):
...
async def test_concurrent_dream_approval_limit(db_engine):
    ...
    async with async_session() as setup_session:
        ...
        student = Student(
            institution_id=inst.id,
            roll_number="DREAM_STD",
            source_type="SYSTEM",
            source_reference="seed",
        )
        c1 = Company(
            institution_id=inst.id, code="C1", source_type="SYSTEM", source_reference="seed"
        )
        c2 = Company(
            institution_id=inst.id, code="C2", source_type="SYSTEM", source_reference="seed"
        )
    ...
    results = await asyncio.gather(
        approve_dream(decl1_id), approve_dream(decl2_id), return_exceptions=True
    )
