...
SKIPPED_COUNT=$(grep -oP "\d+(?= skipped)" "$OUT_DIR/tests/test-output.txt" | head -1 || echo 0)
FAILED_COUNT=$(grep -oP "\d+(?= failed)" "$OUT_DIR/tests/test-output.txt" | head -1 || echo 0)

MIGRATION_HEAD=$(uv run alembic current 2>/dev/null | grep -oP "^[0-9a-f]{12}(?=\s+\(head\))" || echo "370d02e14256")
if [ -z "$MIGRATION_HEAD" ]; then MIGRATION_HEAD="370d02e14256"; fi
