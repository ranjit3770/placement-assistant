...
class Document(TenantRow):
    ...
    constraints = (
        UniqueConstraint("institution_id", "storage_key"),
        CheckConstraint("content_hash ~ '^[0-9a-f]{64}$'", name="valid_hash"),
        choices("media_type", "application/pdf text/markdown text/plain"),
    )
...
class SourceLocator(TenantRow):
    ...
    constraints = (
        ...
        CheckConstraint("page IS NULL OR page > 0", name="positive_page"),
        CheckConstraint(
...
class PolicyVersion(Revision, TenantRow):
    ...
    constraints = (
        *revision_constraints("policy_id", "policies"),
        scoped_fk("academic_year_id", "academic_years"),
        choices("status", "DRAFT PROCESSING REVIEW APPROVED ACTIVE ARCHIVED"),
        CheckConstraint(
            "schema_version > 0 AND jsonb_typeof(definition) = 'object'", name="definition_shape"
        ),
        UniqueConstraint("institution_id", "academic_year_id", "scope", "id"),
    )
...
class PolicyEvent(Revision, TenantRow):
    ...
    reason: Mapped[str]
    constraints = (
        *revision_constraints("policy_version_id", "policy_versions"),
        choices("status", "DRAFT PROCESSING REVIEW APPROVED ACTIVE ARCHIVED"),
    )
...
class PolicyRule(TenantRow):
    ...
    constraints = (
        ...
        UniqueConstraint("institution_id", "policy_version_id", "code"),
...
class PolicyActivation(TenantRow):
    ...
    constraints = (
        ForeignKeyConstraint(
            ["institution_id", "academic_year_id", "scope", "policy_version_id"],
            [
                "policy_versions.institution_id",
                "policy_versions.academic_year_id",
        ...
        ExcludeConstraint(
            ...
            name="ex_policy_activation_overlap",
            using="gist",
        ),
    )
...
class Requirement(TenantRow):
    ...
    constraints = (
        ...
        UniqueConstraint("institution_id", "opportunity_id"),
    )
...
class RequirementVersion(Revision, TenantRow):
    ...
    constraints = (
        *revision_constraints("requirement_id", "company_requirements"),
        scoped_fk("compensation_id", "compensation_terms"),
        UniqueConstraint("institution_id", "requirement_id", "id"),
    )
...
class RequirementActivation(TenantRow):
    ...
    constraints = (
        ForeignKeyConstraint(
            ["institution_id", "requirement_id", "requirement_version_id"],
            [
                "company_requirement_versions.institution_id",
                "company_requirement_versions.requirement_id",
                "company_requirement_versions.id",
            ],
        ),
        CheckConstraint("ends_at IS NULL OR ends_at > starts_at", name="interval_bounds"),
        ExcludeConstraint(
            ...
            ("requirement_id", "="),
            (text("tstzrange(starts_at, ends_at, '[)')"), "&&"),
            name="ex_requirement_activation_overlap",
            using="gist",
        ),
    )
...
class Criterion(TenantRow):
    ...
    constraints = (
        ...
        UniqueConstraint("institution_id", "requirement_version_id", "code"),
        choices("applicability", "REQUIRED OPTIONAL NOT_APPLICABLE UNKNOWN"),
        choices("operator", "> >= < <= = != IN NOT_IN BETWEEN"),
        CheckConstraint(
            "(applicability IN ('REQUIRED','OPTIONAL') AND operator IS NOT NULL AND operand IS NOT NULL AND jsonb_typeof(operand) = 'object') OR (applicability IN ('UNKNOWN','NOT_APPLICABLE') AND operator IS NULL AND operand IS NULL)",
            name="operand_presence",
        ),
    )
...
class RequirementMember(TenantRow):
    ...
    constraints = (
        scoped_fk("criterion_id", "requirement_criteria"),
        scoped_fk("department_id", "departments"),
        scoped_fk("degree_id", "degrees"),
        UniqueConstraint("institution_id", "criterion_id", "value"),
        CheckConstraint("department_id IS NULL OR degree_id IS NULL", name="single_member_type"),
    )
...
class RuleSource(TenantRow):
    ...
    constraints = (
        scoped_fk("rule_id", "policy_rules"),
        scoped_fk("locator_id", "policy_source_locators"),
        UniqueConstraint("institution_id", "rule_id", "locator_id"),
    )
...
class RequirementSource(TenantRow):
    ...
    constraints = (
        scoped_fk("criterion_id", "requirement_criteria"),
        scoped_fk("locator_id", "policy_source_locators"),
        UniqueConstraint("institution_id", "criterion_id", "locator_id"),
    )
